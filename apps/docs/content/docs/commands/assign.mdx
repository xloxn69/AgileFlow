...
**Q: Can I assign to someone who's already at WIP limit?**
A: Yes, but it's not recommended. Check `/agileflow:board` to see who has capacity.

**Q: What if I assign wrong owner?**
A: Simply reassign: `/agileflow:assign STORY=US-0042 NEW_OWNER=AG-API`

**Q: Should I assign immediately upon creation?**
A: Best practice is yes - assign within the creation command or right after with `/assign`.

**Q: Can I assign to multiple owners?**
A: No, one story = one owner. For pair programming, assign to primary and note pairing in description.

**Q: What's the difference between assign and status?**
A:
- **assign** - Changes owner AND optionally status
- **status** - Changes status only, owner unchanged

Use **assign** when changing ownership, **status** when just updating progress.

**Q: How do I know what owners are available?**
A: Use `/agileflow:board` to see current team members and their workload.
