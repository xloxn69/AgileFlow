...
Creates agent profile markdown at:
```
docs/02-practices/prompts/agents/agent-<AGENT_ID>.md
```
...
```
AG-UI gets stories tagged: frontend, ui, ux, design
AG-API gets stories tagged: backend, api, database
AG-CI gets stories tagged: ci, devops, automation
```
