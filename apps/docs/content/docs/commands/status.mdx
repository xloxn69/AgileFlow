...
**Q: Can I change status directly?**
A: Yes, use the `STATUS` parameter with any valid status value.

**Q: What if I want to assign the story too?**
A: Use `/agileflow:assign` which handles both assignment and status updates together.

**Q: How do I notify specific agents?**
A: Use the `TO` parameter: `/agileflow:status STORY=US-0042 STATUS=done TO=AG-REVIEW`
