...
**Q: How is a blocker detected?**
A: Blockers are identified by:
- Stories with `status: "blocked"` in status.json
- Stories waiting on incomplete dependencies
- Agents at WIP limit with ready stories waiting
- Stories unchanged for >14 days

**Q: How are blocker types determined?**
A: Types are inferred from:
- `blocked_by` field text (contains keywords like "API", "approval", "unclear")
- Bus messages with blocker comments
- Story dependency chains
- Epic context

**Q: Can I create a spike story to unblock research?**
A: Yes, use `/story-new` to create a spike: `/story-new EPIC=EP-0012 TYPE=spike TITLE="Research modern deployment platforms"`

**Q: What's the difference between blocked and WIP limit?**
A: **Blocked** means can't proceed (external dependency). **WIP limit** means agent is at capacity and should finish current work before starting more.
