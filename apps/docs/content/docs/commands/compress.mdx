...
- 1 token ≈ 4 characters/bytes (approximate)
- 100KB file ≈ 25,000 tokens
- Target: < 100KB to stay well under limits

## FAQ

**Q: Does compression delete story files?**
A: No. Only status.json is modified. Story markdown in docs/06-stories/ is untouched.

**Q: Can I undo compression?**
A: Yes. A backup is created automatically. Just restore: `cp docs/09-agents/status.json.backup docs/09-agents/status.json`

**Q: When should I compress?**
A: When status.json exceeds 25KB, or monthly as maintenance. Combine with archival for best results.

**Q: Does this affect agent performance?**
A: Yes, dramatically improves it. Smaller status.json = faster reads, more available context for agents.

**Q: What about git history?**
A: All changes are committed. Previous versions are in git history if needed.

**Q: Can I customize which fields to keep?**
A: Yes, edit the compression script to keep additional fields if needed for your workflow.
