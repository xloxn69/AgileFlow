...
```mermaid
flowchart LR
    A[Take Screenshot] --> B[login-form.png]
    B --> C{Review}
    C -->|Approved| D[verified-login-form.png]
    C -->|Issues Found| E[Fix & Retake]
    E --> B
    D --> F[Loop Continues]
```
...
```mermaid
flowchart TB
    A[Claude Stops] --> B[Run Tests]
    B --> C{Tests Pass?}
    C -->|No| D[Show Test Failures]
    C -->|Yes| E{Visual Mode?}
    E -->|No| F[Mark Complete]
    E -->|Yes| G[Check Screenshots]
    G --> H{All Verified?}
    H -->|Yes| F
    H -->|No| I[Pause for Review]
    I --> J[User Verifies]
    J --> K[Continue Loop]
    D --> K
```
