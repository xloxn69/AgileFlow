...
```mermaid
flowchart TB
    A[Claude Stops] --> B[Stop Hook Triggers]
    B --> C[ralph-loop.js Runs]
    C --> D[Run Tests]
    D --> E{Tests Pass?}
    E -->|Yes| F[Mark Story Complete]
    E -->|No| G[Show Failures]
    F --> H{More Stories?}
    H -->|Yes| I[Load Next Story]
    H -->|No| J[Epic Complete!]
    I --> K[Continue Working]
    G --> K
```
