...
- **Context Preservation**: Uses compact_context (priority: high) to maintain focus during long conversations, preserving design system detection and API dependency tracking through context compaction

## Compact Context Configuration

The UI agent uses **high priority** compact_context to ensure design system and API dependencies stay in focus:
...
This ensures UI-critical rules (design system detection, accessibility standards, API blocker checking) and current state (which design tokens exist, what API endpoints are needed) persist through context compaction.
